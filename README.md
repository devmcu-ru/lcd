...
Библиотека машинной графики для микроконтроллеров, раземр ОЗУ которых не позволяет полноценно работать с фреймбуфером.
Библиотека не привязана к какой-либо арихетктуре микроконтроллера, вместо этого драйверы используютс колбеки -
функции который реализует пользователь и передает при инициализации драйвера.

Такой подход позволяет максимально изспользовать периферию (SPI, DMA, ...) микроконтроллеров и при этом код остается переносимым
...
void lcd_spi_transmit(uint8_t *tx, size_t size)
{
  ...
  // передаем данные в блокируещем режиме
  if (size <= 4) {
    while (size --) {
...
Слой графики (Canvas) позволяет выводить базовые примиты на экран:
```c
// Инициализация
